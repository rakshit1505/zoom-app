  ...
  module Actions
    module Meeting
      ...
      def recordings(*args)
        ...
        Utils.process_datetime_params!(:start_time, options)
        Utils.parse_response self.class.get("/meetings/{meeting_id}/recordings", body: options.except(:meeting_id).to_json, headers: request_headers)
      end
      ...
      def meetings
        options = Zoom::Params.new(Utils.extract_options!(args))
        Utils.parse_response self.class.put("/meetings/", body: options, headers: request_headers)
      end
